        ...
        header
          ul.counts
            li.count
              | 共计：{{ meeting.all }}人
            li.count
              | 已回复：{{ meeting.allRead }}人
            li.count
              | 未回复：{{ meeting.all - meeting.allRead }}人
          .filter
            | 筛选
            .select
...
<script>
  ...
  export default {
    ...
    created () {
      this.resetPage()
      this.$set(this.meeting, 'meetingId', meetingId)
      this.loadData()
      this.getCopySet()
      this.initReplyCount(meetingId)
//      this.notice = LocalStorage.getLocalData()[LocalStorage.NOTICE]
    ...
    methods: {
      ...filters,
      ...mapActions(['resetPage', 'loadReceivedMeetingUser', 'loadGetCopySet', 'loadExportMeetingUser', 'loadReplyCount', 'goBack']),
      loadData () {
        this.meeting.id = meetingId
        let keyParam = this.meeting
        this.loadReceivedMeetingUser(keyParam).then((data) => {
          let list = []
          let number = 1
      ...
      getCopySet () {
        this.loadGetCopySet(this.meeting.meetingId).then((data) => {
      ...
      searchTypeSelect (data) {
        ...
        if (data === '-1') {
          this.meeting.noticeCopies = null
        } else {
          this.meeting.noticeCopies = [{'copyContentId': data}]
        }
        this.loadData()
      },
      exportUserList () {
        const newTab = window.open('about:blank')
        this.loadExportMeetingUser(this.meeting).then((url) => {
          newTab.location.href = url
        })
      },
      initReplyCount (meetingId) {
        this.loadReplyCount(meetingId).then((data) => {
          this.meeting = data
        })
      }
    }
