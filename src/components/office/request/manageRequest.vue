          ...
          el-table(:data='tableData', :border="true", row-class-name="notices-row")
            ...
            el-table-column(label-class-name='notices-header', align='center', width="200", label='操作', fixed="right")
              template(slot-scope='scope')
                el-button.btn-edit(type='text', size='small',@click="edit(scope.row.requestId)")
                  | 查看详情
                el-button.btn-del(type='text', size='small',@click="deleted(scope.row.requestId)")
                  | 删除
        .bar-page
...
<script>
  ...
  export default {
    ...
    methods: {
      ...filters,
      ...mapActions(['resetPage', 'loadPublishRequests', 'deletedRequest']),
      loadData () {
        let keyParam = {
      ...
      deleted (requestId) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.deletedRequest(requestId).then((data) => {
            this.loadData()
          })
        }).catch(() => {
        })
      ...
      edit (requestId) {
        this.$router.push(`create/${requestId}`)
      }
    }
  }
</script>
