<template lang="pug">
  #createRequest.wrapper-content
    c-header.wrapper-createRequest-header(:menuNames="menuNames")
      .goback(slot='right', v-if="requestId")
            ...
            .row
              header
                span.must
                  | *
                | 事由
              c-input(:disabled="requestId", :outClassName="{'has-content': request.title}", placeholder="请输入请示的标题（40字以内）", :max="40", v-model="request.title")
            .row
              header
                span.must
                  | *
                | 具体内容
              c-input(:disabled="requestId", :outClassName="{'has-content': request.content}", placeholder="请输入请示的内容（200字以内）", :max="200", v-model="request.content")
            .row
              c-upload-attchment(ref="c-upload-attchment", :hiddenBtn="requestId")
          .wrapper-btn-bar(v-if="!requestId")
        ...
        .right
          ...
          ul.peoples
            li.people(v-for = "approve in approves")
              .info
                ...
                .name
                  | {{ approve.name }}
              .nextstep(v-if="!requestId")
                img(src="../common/icon/icon-nextstep.png")
              .del(v-if="!requestId", @click="delSelectedPeople(approve, 1)")
                | +
            li.add(v-if="approves.length < 5 && !requestId")
          ...
          ul.peoples
            li.people(v-for = "approve in copies")
              ...
              .nextstep(v-if="!requestId")
                img(src="../common/icon/icon-nextstep.png")
              .del(v-if="!requestId", @click="delSelectedPeople(approve, 2)")
                | +
            li.add(v-if="copies.length < 5 && !requestId")
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      if (this.requestId) {
        // 查看编辑
        this.loadSendDetailsData(requestId)
        this.menuNames[0].name = '请示详情'
      } else {
        this.btnName = '发布'
      }
    },
    ...
    methods: {
      ...filters,
      ...mapActions(['getCurrentDeptMembers', 'createRequest', 'loadRequestById2', 'goBack', 'workUnread']),
      selectMembers (type) {
        let checkList = []
        if (type === 1) {
      ...
      publish () {
        this.request.approves = this.approves
        this.request.copies = this.copies
        this.request.files = this.cUploadAttchment.getFileList()
        if (!this.checkCanCreate()) {
          return
        }
        this.loadingInstance = Loading.service({text: `${this.btnName}中，请耐心等待……`})
        this.createRequest(this.request, 0).then((data) => {
      ...
      checkCanCreate () {
        if (this.request.title === '' || this.request.content === '') {
        ...
        if (this.request.approves.length === 0) {
      ...
      loadSendDetailsData (requestId) {
        this.loadRequestById2(requestId).then((data) => {
          let approves = []
          let copies = []
          ...
          this.cUploadAttchment.setFileList(data.files)
