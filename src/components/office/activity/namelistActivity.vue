        ...
        header
          ul.counts
            li.count
              | 共计：{{ activity.all }}人
            li.count
              | 已回复：{{ activity.allRead }}人
            li.count
              | 未回复：{{ activity.all - activity.allRead }}人
          .filter
            | 筛选
            .select
...
<script>
  ...
  export default {
    ...
    created () {
      this.resetPage()
      this.$set(this.activity, 'activityId', activityId)
      this.loadData()
      this.initReplyCount(activityId)
//      this.notice = LocalStorage.getLocalData()[LocalStorage.NOTICE]
    ...
    methods: {
      ...
      loadData () {
        ...
        let keyParam = this.activity
        this.loadReceivedActivityUser(keyParam).then((data) => {
          let list = []
          let number = 1
      ...
      searchTypeSelect (data) {
        ...
        if (data === '-1') {
          this.activity.noticeCopies = null
        } else {
          this.activity.noticeCopies = [{'copyContentId': data}]
        }
        this.loadData()
      },
      exportUserList () {
        const newTab = window.open('about:blank')
        this.loadExportActivityUser(this.activity).then((url) => {
          newTab.location.href = url
        })
      },
      initReplyCount (activityId) {
        this.loadReplyCount(activityId).then((data) => {
          this.activity = data
        })
      }
    }
