          ...
          li.notice(v-for="(approval,index) in approvals")
            ...
            .wrapper-notice-content
              header.borderBttom1px(@mouseover="addEvent")
                ...
                .status(:class="{'on': approval.statusText === '已处理'}")
                  | {{ approval.statusText }}
              article(@mouseover="removeEvent")
                ...
                .info
                  ...
                  .content
                    .name
                      | {{ approval.name }}
                    .status-request(:class="{'on': approval.agreeType === 0 || approval.agreeType === 1}")
                      | {{approval.agreeText}}
                .request-content
                  ul
                    li
                    ...
                    li(v-if="approval.attachments.length > 0")
                      label
                        | 附件：
              ...
              footer(@mouseover="addEvent")
                .btn-bar
                  .btn.btn-ok(@click="sendMsg('同意理由',approval, index)",v-if="approval.agreeType === 1")
                    | 同意
                  .btn.btn-no(@click="sendMsg('拒绝理由',approval, index)",v-if="approval.agreeType === 1")
                    | 拒绝
                  .btn.btn-comment(@click="sendMsg('评论',approval, index)",v-if="approval.agreeType === 2 || approval.agreeType === 3")
                    | 评论
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...filters,
      ...mapActions(['resetPage', 'loadReceivedApprovals', 'loadReadCount', 'loadApproval2', 'loadComment2', 'loadApprovalById']),
      setNotices (list) {
        this.approvals = list
        this.loadData = null
        this.$nextTick(() => {
      ...
      loadData (e = {}) {
        ...
        this.loadReceivedApprovals(keyParam).then((data) => {
          this.getPage.hasNextPage = data.total !== this.approvals.length
          this.getPage.page += 1
          for (let item of data.list) {
            this.approvals.push(this.transferData(item))
          }
          this.$nextTick(() => {
            this._calcNoticesWidth()
            e.loading = false
      ...
      _calcNoticesWidth () {
        ...
        width -= 20
        this.$refs['e-approvals'].style.width = width + 'px'
      },
      resetChangePage () {
        this.getPage.page = 1
        this.approvals = []
      ...
      getApprovalById (approval, index) {
        this.loadApprovalById(approval.approvalId).then((data) => {
          this.approvals.splice(index, 1, this.transferData(data))
        }).catch((data) => {
          this.$alert(data.msg)
        })
      },
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              if (title === '同意理由') {
                approval.agreeType = 2
                this.loadApproval2(approval).then((data) => {
      ...
      updateBadgeCount () {
        this.getShareData.unReadCount.approval.check.value -= 1
      }
    }
  }
</script>
