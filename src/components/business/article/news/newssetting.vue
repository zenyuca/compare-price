          ...
          <div class="public-form-content">
            <el-form :model="type" ref="info" label-width="120px" class="public-form public-form-setting">
              <el-form-item :label="index===0?'类型配置':''"
                v-for="(val,key,index) in type.info" :key="key">
                <el-input placeholder="请输入..."  :value="val" @input="(v)=>type.info[key] = v">
                  <img style="cursor: pointer" slot="append" src="./icon/icon-del.png" class="close" alt="close" @click="del(key)">
                </el-input>
              </el-form-item>
              <el-form-item>
                <div class="btn-add" @click="$set(type.info,new Date().getTime(),'')">+ 添加</div>
              </el-form-item>
              <div class="posb">
...
//  import { Loading } from 'element-ui'
  ...
  export default {
    ...
    methods: {
      ...
      del (key) {
        this.$delete(this.type.info, key)
      },
      submitForm () {
        let valid = true
        for (let key in this.type.info) {
          if (!this.type.info[key]) {
            valid = false
          }
        }
