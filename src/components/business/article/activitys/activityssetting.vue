          ...
          <div class="public-form-content">
            <el-form :model="type" ref="info" label-width="120px" class="public-form public-form-setting">
              <el-form-item :label="index===0?'类型配置':''"
                v-for="(val,key,index) in type.activity" :key="key">
              ...
              <el-form-item>
                <div class="btn-add" @click="$set(type.activity,new Date().getTime(),'')">+ 添加</div>
              </el-form-item>
              <div class="posb">
...
//  import { Loading } from 'element-ui'
  ...
  export default {
    ...
    methods: {
      ...
      submitForm () {
        let valid = true
        for (let key in this.type.activity) {
          if (!this.type.activity[key]) {
            valid = false
          }
        }
